...
import { AxiosCustomError, ICreateUser } from "../../helper/interface";
...
import { addUpdateUserValidation } from "../../helper/validation";

interface Props {
  onAction: () => void;
  onCancel: () => void;
  initialValue: ICreateUser;
  userId: string;
}
    ...
    async (data: ICreateUser, { resetForm }: FormikHelpers<ICreateUser>) => {
      ...
      try {
        const payload = {
          ...
          ...(data.updatePassword ? { password: data.password } : {}),
        };
        const response = await dataService.put(API.updateUser, payload);
