...
export const addUpdateUserValidation = Yup.object({
  name: validateString.required("Name is required"),
  userName: validateString.required("User name is reqiured").matches(/^\S*$/g, "Blank spaces not allowed"),
  ...
  confirmPassword: validateString.when("updatePassword", (validatePassword, schema) => {
